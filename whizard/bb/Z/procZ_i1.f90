...
module opr_procZ_i1
  ...
  data table_spin_states(:,   1) / -1, -1, -1 /
  data table_spin_states(:,   2) / -1, -1,  0 /
  data table_spin_states(:,   3) / -1, -1,  1 /
  data table_spin_states(:,   4) / -1,  1, -1 /
  data table_spin_states(:,   5) / -1,  1,  0 /
  data table_spin_states(:,   6) / -1,  1,  1 /
  data table_spin_states(:,   7) /  1, -1, -1 /
  data table_spin_states(:,   8) /  1, -1,  0 /
  data table_spin_states(:,   9) /  1, -1,  1 /
  data table_spin_states(:,  10) /  1,  1, -1 /
  data table_spin_states(:,  11) /  1,  1,  0 /
  data table_spin_states(:,  12) /  1,  1,  1 /

  integer, dimension(n_prt,n_flv), save, protected :: table_flavor_states
  data table_flavor_states(:,   1) /  11, -11,  23 / ! e- e+ Z

  integer, dimension(n_cindex,n_prt,n_cflow), save, protected :: table_color_flows
  data table_color_flows(:,:,   1) / 0,0,  0,0,  0,0 /

  logical, dimension(n_prt,n_cflow), save, protected :: table_ghost_flags
  data table_ghost_flags(:,   1) / F,  F,  F /
  ...
  integer, save :: hel_finite = n_hel
    ...
    type(momentum) :: p1, p2, p3
  type thread_local_data
    type(spinor) :: owf_fl1_p1
    type(conjspinor) :: owf_fl1b_p2
    type(vector) :: owf_fz_p3
    complex(kind=default) :: oks_fl1_fl1b_fz
...
contains
  ...
  pure function md5sum ()
    ...
    md5sum = "3DA9896BF6C5C03ED0BD735E1A6BE7BA"
  end function md5sum
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(s, h, tld) SCHEDULE(STATIC)
    do hi = 1, hel_finite
      ...
      tld%owf_fz_p3 = conjg (eps (mass(23), p3, s(3)))
      call compute_brakets_0001 (tld)
      amp(1,1,h) = tld%oks_fl1_fl1b_fz
    end do
!$OMP END PARALLEL DO
  end subroutine calculate_amplitudes
  subroutine compute_brakets_0001 (tld)
  type(thread_local_data), intent(inout) :: tld
      tld%oks_fl1_fl1b_fz = 0
      tld%oks_fl1_fl1b_fz = tld%oks_fl1_fl1b_fz + ( &
         + f_fva(gnclep(1),gnclep(2),tld%owf_fl1b_p2,tld%owf_fz_p3))*tld%owf_fl1_p1
      ! tld%oks_fl1_fl1b_fz = tld%oks_fl1_fl1b_fz ! 1 vertices, 0 propagators
