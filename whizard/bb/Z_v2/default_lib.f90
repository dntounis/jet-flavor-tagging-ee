...
function default_lib_md5sum () result (md5sum)
  implicit none
  character(32) :: md5sum
  md5sum = 'E6766CC17AE117632D072D32EC2386AE'
...
subroutine default_lib_get_process_id_ptr (i, cptr, len) bind(C)
  ...
  case (1);  call default_lib_string_to_array ('procZ_i1', a)
  end select
  if (allocated (a)) then
     cptr = c_loc (a)
     len = size (a)
...
subroutine default_lib_get_md5sum_ptr (i, cptr, len) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_md5sum => md5sum
  implicit none
  interface
     function default_lib_md5sum () result (md5sum)
       character(32) :: md5sum
...
function default_lib_get_openmp_status (i) result (openmp_status) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_openmp_supported => openmp_supported
  implicit none
  integer(c_int), intent(in) :: i
  logical(c_bool) :: openmp_status
  select case (i)
  case (1);  openmp_status = procZ_i1_openmp_supported ()
  end select
...
function default_lib_get_n_in (pid) result (n_in) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_n_in => number_particles_in
  implicit none
  integer(c_int), intent(in) :: pid
  integer(c_int) :: n_in
  select case (pid)
  case (1);  n_in = procZ_i1_n_in ()
  end select
...
function default_lib_get_n_out (pid) result (n_out) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_n_out => number_particles_out
  implicit none
  integer(c_int), intent(in) :: pid
  integer(c_int) :: n_out
  select case (pid)
  case (1);  n_out = procZ_i1_n_out ()
  end select
...
function default_lib_get_n_flv (pid) result (n_flv) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_n_flv => number_flavor_states
  implicit none
  integer(c_int), intent(in) :: pid
  integer(c_int) :: n_flv
  select case (pid)
  case (1);  n_flv = procZ_i1_n_flv ()
  end select
...
function default_lib_get_n_hel (pid) result (n_hel) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_n_hel => number_spin_states
  implicit none
  integer(c_int), intent(in) :: pid
  integer(c_int) :: n_hel
  select case (pid)
  case (1);  n_hel = procZ_i1_n_hel ()
  end select
...
function default_lib_get_n_col (pid) result (n_col) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_n_col => number_color_flows
  implicit none
  integer(c_int), intent(in) :: pid
  integer(c_int) :: n_col
  select case (pid)
  case (1);  n_col = procZ_i1_n_col ()
  end select
...
function default_lib_get_n_cin (pid) result (n_cin) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_n_cin => number_color_indices
  implicit none
  integer(c_int), intent(in) :: pid
  integer(c_int) :: n_cin
  select case (pid)
  case (1);  n_cin = procZ_i1_n_cin ()
  end select
...
function default_lib_get_n_cf (pid) result (n_cf) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_n_cf => number_color_factors
  implicit none
  integer(c_int), intent(in) :: pid
  integer(c_int) :: n_cf
  select case (pid)
  case (1);  n_cf = procZ_i1_n_cf ()
  end select
...
subroutine default_lib_set_flv_state_ptr (pid, flv_state, shape) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_flv_state => flavor_states
  implicit none
  integer(c_int), intent(in) :: pid
  ...
  case (1)
     ...
     call procZ_i1_flv_state (flv_state_tmp)
...
subroutine default_lib_set_hel_state_ptr (pid, hel_state, shape) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_hel_state => spin_states
  implicit none
  integer(c_int), intent(in) :: pid
  ...
  case (1)
     ...
     call procZ_i1_hel_state (hel_state_tmp)
     forall (i=1:shape(1), j=1:shape(2)) 
...
subroutine default_lib_set_col_state_ptr (pid, col_state, ghost_flag, shape) bind(C)
  use iso_c_binding
  use opr_procZ_i1, only: procZ_i1_col_state => color_flows
  implicit none
  integer(c_int), intent(in) :: pid
  ...
  allocate (col_state_tmp (shape(1), shape(2), shape(3)))
     ...
     call procZ_i1_col_state (col_state_tmp, ghost_flag_tmp)
...
subroutine default_lib_set_color_factors_ptr (pid, cf_index1, cf_index2, color_factors, shape) bind(C)
  ...
  use kinds
  use omega_color
  use opr_procZ_i1, only: procZ_i1_color_factors => color_factors
  implicit none
  integer(c_int), intent(in) :: pid
  ...
  case (1)
     allocate (cf (shape(1)))
     call procZ_i1_color_factors (cf)
...
subroutine default_lib_get_fptr (pid, fid, fptr) bind(C)
  ...
  interface
     subroutine procZ_i1_init (par, scheme) bind(C)
       import
       real(c_default_float), dimension(*), intent(in) :: par
       integer(c_int), intent(in) :: scheme
     end subroutine procZ_i1_init
  end interface
  interface
     subroutine procZ_i1_update_alpha_s (alpha_s) bind(C)
       import
       real(c_default_float), intent(in) :: alpha_s
     end subroutine procZ_i1_update_alpha_s
  end interface
  interface
     subroutine procZ_i1_reset_helicity_selection (threshold, cutoff) bind(C)
       import
       real(c_default_float), intent(in) :: threshold
       integer(c_int), intent(in) :: cutoff
     end subroutine procZ_i1_reset_helicity_selection
  end interface
  interface
     subroutine procZ_i1_is_allowed (flv, hel, col, flag) bind(C)
       import
       integer(c_int), intent(in) :: flv, hel, col
       logical(c_bool), intent(out) :: flag
     end subroutine procZ_i1_is_allowed
  end interface
  interface
     subroutine procZ_i1_new_event (p) bind(C)
       import
       real(c_default_float), dimension(0:3,*), intent(in) :: p
     end subroutine procZ_i1_new_event
  end interface
  interface
     subroutine procZ_i1_get_amplitude (flv, hel, col, amp) bind(C)
       import
       integer(c_int), intent(in) :: flv, hel, col
       complex(c_default_complex), intent(out) :: amp
     end subroutine procZ_i1_get_amplitude
  end interface
  select case (pid)
  case (1)
     select case (fid)
     case (1);  fptr = c_funloc (procZ_i1_init)
     case (2);  fptr = c_funloc (procZ_i1_update_alpha_s)
     case (3);  fptr = c_funloc (procZ_i1_reset_helicity_selection)
     case (4);  fptr = c_funloc (procZ_i1_is_allowed)
     case (5);  fptr = c_funloc (procZ_i1_new_event)
     case (6);  fptr = c_funloc (procZ_i1_get_amplitude)
     end select
  end select
